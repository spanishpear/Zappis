...
export class WireManager {
  ...
  private pointFromGridPosition(gridPos: GridPosition): Point {
    return new Point(gridPos.x * this.GRID_SIZE, gridPos.y * this.GRID_SIZE);
  }
  ...
  public calculateRoute(start: Port, end: Port): Wire {
    ...
    if (this.creationState.waypoints.length > 0) {
      const wire = this.calculateRouteWithWaypoints(
        start,
        end,
        this.creationState.waypoints,
      );
    ...
    const wirePoints: WirePoint[] = pathResult.map(
      (pos): WirePoint => ({
        position: this.pointFromGridPosition(pos),
        type:
          pos === pathResult[0]
            ? 'component'
            : pos === pathResult[pathResult.length - 1]
              ? 'component'
              : 'bend',
        connectionType:
          pos === pathResult[0]
    ...
    for (let i = 0; i < wirePoints.length - 1; i++) {
      ...
      if (current && next) {
        segments.push({
          ...
          direction:
            current.position.x === next.position.x ? 'vertical' : 'horizontal',
        });
      }
    }
    ...
    return (
      allPorts.find((port) => {
        const distance = Math.sqrt(
          (port.position.x - point.x) ** 2 + (port.position.y - point.y) ** 2,
        );
  ...
  public updatePreview(point: Point): void {
    ...
    const previewWire = this.calculateRoute(this.creationState.startPort, {
      id: 'preview',
      position: point,
      direction: 'west',
      type: 'input',
    ...
    while (current < path.length - 1) {
      ...
      for (let i = current + 2; i < path.length; i++) {
        ...
        if (
          currentPoint &&
          testPoint &&
          this.canDirectConnect(currentPoint, testPoint)
        ) {
          furthest = i;
        }
      }
